  ...
  return (
    <section className={`flex items-center justify-center min-h-screen p-4 ${dark ? "bg-gray-900 text-white" : "bg-gray-100 text-black"}`}>
      <div className={`p-8 rounded-lg shadow-md w-full max-w-md ${dark ? "bg-gray-800" : "bg-[#81D8D0]"}`}>
        <h2 className={`text-3xl font-bold mb-6 text-center ${dark ? "text-[#81D8D0]" : "text-black"}`}>Cadastro</h2>
        <form onSubmit={handleSubmit} className="flex flex-col">
          ...
          <div className="mt-4 text-center">
            <p>Já tem uma conta? <Link to="/login" className={` ${dark ? "text-white font-bold hover:underline" : "text-black font-bold hover:underline"}`}>Faça login</Link></p>
          </div>
        </form>
      </div>
    </section>
